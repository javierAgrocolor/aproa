...
/**
 * This is the model class for table "Origen_Cooperativas".
 *
 * @property string $id
 ...
 */
class OrigenCooperativas extends \yii\db\ActiveRecord
{
    /**
     * @inheritdoc
     */
    ...
    public function leerYears(){
        ...
        $query ->select(['distinct datepart(year,fecha) as year'])
                ->from('Origen_Cooperativas')
                ->orderBy('year');
        $rows = $query->all(OrigenCooperativas::getDb());
        /*$query -> select('Precio')
    ...
    public function leerSemanas($fechaInicial){
        ...
        $query ->select(['distinct fecha, convert(varchar(10),fecha,103) as fechaCorta, (datepart(DY, datediff(d, 0, fecha) / 7 * 7 + 3)+6) / 7 as week'])
                ...
                ->groupBy('fecha')
                ->orderBy('Origen_Cooperativas.fecha');
        $rows = $query -> all(OrigenCooperativas::getDb());
        return $rows;
    }
    ...
    public function generarCondiciones($productos, $fechaInicial, $fechaFinal){
        ...
        $condiciones = "Origen_Cooperativas.cod_presentacion = 0";
        ...
        if ($fechaInicial != ""){
            $condiciones .= " and Origen_Cooperativas.fecha >= convert(datetime,'".$fechaInicial."')";
        }

        if ($fechaFinal != ""){
            $condiciones .= " and Origen_Cooperativas.fecha <= convert(datetime,'".$fechaFinal."')";
        }
    ...
    public function generarCondicionesSemanales($productos, $semanas, $fechaInicial, $fechaFinal, $anio){
        ...
        if ($anio != ""){
            $condiciones .= " and Origen_Cooperativas.fecha >= '01-08-".$anio."'";
        }

        if ($anio != ""){
            $condiciones .= " and Origen_Cooperativas.fecha <= '31-07-".$nextYear."'";
        }
    ...
    public function generarCondSemanas($semanas, $condiciones){
        ...
        if (isset($semanas)){
            ...
            $condiciones .= " and (datepart(DY, datediff(d,0,Origen_Cooperativas.fecha)/7*7+3)+6)/7 in (";
            ...
            $condiciones .= " and DATEPART(year, Origen_Cooperativas.fecha) in (";
            $contador = 0;
            foreach ($semanas as $semana){
                if ($contador === 0){
    ...
    public function generarCondProductos($productos, $condiciones){
        ...
        if (isset($productos)){
            $condiciones .= " and Origen_Cooperativas.cod_producto in (";
    ...
    public function consultarTodos($condiciones){
        ...
        $query->select(['*'])
                ->from('Origen_Cooperativas')
                ->innerJoin('Origen', 'Origen.codigo_origen = Origen_Cooperativas.cod_origen')
                ->innerJoin('Localizacion', 'Localizacion.codigo_localizacion = Origen_Cooperativas.cod_localizacion')
                ->innerJoin('Producto', 'Producto.codigo_producto = Origen_Cooperativas.cod_producto')
                ->where($condiciones)
    ...
    public function consultarMediasSemanales($condiciones){
        $query = new \yii\db\Query();
        //Cambio de datepart(week,Datos_origen.fecha) por (datepart(DY, datediff(d,0,Datos_origen.fecha)/7*7+3)+6)/7
        $query -> select(['producto.producto, Localizacion.Localizacion, origen.origen, Round(avg(precio),2) as preciomedio, (datepart(DY, datediff(d,0,Origen_Cooperativas.fecha)/7*7+3)+6)/7 as Semana', 'DATEPART(year, Origen_Cooperativas.fecha) as anio'])
                -> from('Origen_Cooperativas')
                -> innerJoin('Origen', 'Origen.codigo_origen = Origen_Cooperativas.cod_origen')
                -> innerJoin('Localizacion', 'Localizacion.codigo_localizacion = Origen_Cooperativas.cod_localizacion')
                -> innerJoin('Producto', 'Producto.codigo_producto = Origen_Cooperativas.cod_producto')
                ->where($condiciones)
                ->groupBy(['Producto', 'Localizacion', 'Origen', '(datepart(DY, datediff(d,0,Origen_Cooperativas.fecha)/7*7+3)+6)/7', 'DATEPART(year, Origen_Cooperativas.fecha)'])
    ...
    public function leerProductos(){
        $query = new \yii\db\Query();
        $query -> select('producto.producto, Origen_Cooperativas.cod_producto')
